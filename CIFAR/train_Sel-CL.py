    ...
    for epoch in range(args.initial_epoch, args.epoch + 1):
        ...
        if(epoch<=args.warmup_epoch):
            if(args.warmup_way=='uns'):
                ...
                train_sup(args, scheduler,model,model_ema,uns_contrast,queue,device, train_loader, train_selected_loader, optimizer, epoch,torch.eq(trainNoisyLabels, trainNoisyLabels.t()))   
        else:
            ...
            train_sel(args, scheduler,model,model_ema,uns_contrast,queue,device, train_loader, train_selected_loader, optimizer, epoch,selected_pairs)
